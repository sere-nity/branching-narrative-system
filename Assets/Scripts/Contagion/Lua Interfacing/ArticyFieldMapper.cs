    ...
    public static class ArticyFieldMapper
    {
        ...
        public static List<CheckModifier> GetApplicableModifiers(DialogueEntry entry)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                string expression = Field.LookupValue(entry.fields, variableFieldList[i]);
            
                // Only create CheckModifier if expression exists and is true
                if (!string.IsNullOrEmpty(expression) && Lua.IsTrue(expression))
                {
